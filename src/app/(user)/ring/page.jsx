...
const Page = () => {
  ...
  const { favoriteIds, addFavorite } = useFavorites();
  ...
  const displayedData = React.useMemo(() => {
    ...
    list = list.filter(
      (item) => String(item?.Category?.name || "").toLowerCase() === "nhan",
    );
  ...
  return (
    <div className="flex flex-col items-center mt-6 lg:mt-10 px-4 lg:px-0">
      ...
      {/* MAIN */}
      <div className="w-full max-w-[1200px] flex gap-4 lg:gap-5 items-start">
        {/* ✅ DESKTOP FILTER (CỐ ĐỊNH, KHÔNG STICKY) */}
